...
# resFast <- resNew <- resOld <- rep(Inf, length(to_check))
# for(i in seq_along(to_check)){
#   cat(to_check[i], 'leaves\n')
#   tree <- make_balanced_tree(to_check[i])
#   res <- microbenchmark::microbenchmark(#dendrapply(tree, \(x) x, useFast=FALSE),
#                                         dendrapply(tree, \(x) x, useFast=TRUE),
...
str(dendrapply(dhc21, \(x) x), useFast=FALSE)
